...
Result shapes, links and tables, caching behaviour, session security, failure
diagnostics, XPath extraction, and multi-host crawling are covered in
[docs/tool-reference.md](docs/tool-reference.md).

Each tool also carries full parameter documentation in its own docstring, which
your MCP client displays at call time.
...
[`AGENTS.md`](AGENTS.md) is the working brief for AI coding agents: commands,
the invariants that are easy to break, and how to test something that the unit
suite cannot see.

## Getting Help

- **Something not working?** Start with [Troubleshooting](#troubleshooting), then
  run `uv run crawl4ai-doctor` and call the `ping` tool — it reports whether the
  browser is ready, installing, or unavailable.
- **Bug reports and feature requests:** [open an issue](https://github.com/potterdigital/crawl4ai-mcp/issues).
  Include your crawl4ai version (`check_update` reports it), the tool call you
  made, and what came back.
...
[AGENTS.md](AGENTS.md) if you are pointing a coding agent at this repo.
