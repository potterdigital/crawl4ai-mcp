...
Add the `extract_css` tool function after `extract_structured` (or after `crawl_url` if 04-01 hasn't run yet — either ordering is fine, as long as it's after the helper functions). Signature:
...
3. `test_extraction_tools_are_separate_functions` — import `crawl_url`, `extract_structured` (if available — use try/except for wave-1 parallel safety), and `extract_css`. Assert they are distinct function objects (`crawl_url is not extract_css`).

Import `inspect` for signature introspection.
Import from `crawl4ai_mcp.server`: `extract_css`, `crawl_url`.
Try-import `extract_structured` (may not exist yet if 04-02 runs before 04-01 — handle gracefully with `pytest.importorskip` or conditional test).
  </action>
  <verify>
